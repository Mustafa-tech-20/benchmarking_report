...
from product_planning_agent.async_config import rate_limit_config, interleaved_config
...
class PriorityTaskQueue:
    ...
    async def push(self, priority: int, task: Any) -> bool:
        """Push a task onto the queue. Returns False if queue is full."""
        async with self._lock:
...
class ResourceScheduler:
    ...
    def __init__(self):
        self.limiters: Dict[ResourceType, EnhancedTokenBucket] = {
            ...
            ResourceType.IMAGE_SEARCH: EnhancedTokenBucket(
                rate=rate_limit_config.custom_search_requests_per_second / 2,
        ...
        self._pending_tasks: Dict[str, int] = {}
    ...
    async def schedule_next_task(self) -> Optional[CarTask]:
        ...
        return await self.schedule_next_task()
